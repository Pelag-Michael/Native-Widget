...
restores `RootBorder`. Dragging uses manual mouse-capture move (not `DragMove`): snap-close menu, keep dock
visible, follow cursor in DIP; suppress hover-reopen until pointer leaves so the menu does
not flicker open mid-drag. Launcher action
